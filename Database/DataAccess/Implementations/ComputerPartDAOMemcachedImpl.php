...
class ComputerPartDAOMemcachedImpl implements ComputerPartDAO
{
    ...
    public function getAll(int $offset, int $limit): array
    {
        ...
        // Memcachedからすべてのキーを取得:O(n), n:keys 
        $keys = $memcached->getAllKeys();
        // すべてのキーに対してフィルタリング: O(n)
        $computerPartKeys = array_filter($keys, fn($key) => str_starts_with($key, "ComputerPart_"));
        // キーのソート(quick sort): O(n log n)
        $keys = sort($keys, SORT_STRING);
        // 指定されたオフセットとリミットに基づいてキーをスライス: O(k) (k: limit)
        $selectedKeys = array_slice($computerPartKeys, $offset, $limit);
        // 各キーに対してMemcachedから値を取得し、JSONデコードおよびresultToComputerPartを呼び出し: O(k) (k: limit)
    ...
    {   // O(n log n): dominant in this method
